...
**Simple first → working MVP → visual review → improvements.** Home business: ADMIN + WORKERs, Telegram, phones, one NAS.
...
- **Realtime** after COMMIT via Socket.IO + Redis pub/sub.
- **Flutter** (Riverpod, GoRouter, Dio, Freezed, Drift) for ADMIN and WORKER, **Next.js** for tables/reports/settings.
...
apps/web       Next.js (status, tables, reports, settings)
apps/mobile    Flutter (ADMIN + WORKER, role-based shells)
packages/shared    roles, enums, state machines, registration flow, validation (zod), realtime contracts, money/metres
...
`config` · `common` (context, errors, zod pipe, idempotency) · `prisma` · `redis` · `storage`+`files` · `audit` · `events` (bus + Socket.IO gateway) · `auth` · `notifications` (outbox) · `registration` (bot logic) · `workers` · `collateral` · later: `catalog`, `stock`, `assignments`, `deliveries`, `quality`, `finance`, `sales`, `qr`, `map`.
