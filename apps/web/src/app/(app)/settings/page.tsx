      ...
      {canManage && history.data && history.data.items.length > 0 && (
        <div className="space-y-1 text-sm">
          ...
          {history.data.items.map((h) => (
            <div key={h.id} className="flex justify-between border-b border-border py-1 text-muted">
              <span>{h.previousRatePerKit ? `${formatUzs(h.previousRatePerKit)} → ${formatUzs(h.ratePerKit)}` : `${formatUzs(h.ratePerKit)} · начальная`}</span>
              <span>{h.changedBy ?? '—'} · {formatDate(h.createdAt)}</span>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <section className="space-y-3">
      ...
      <Card className="max-w-md space-y-3">
        <Input placeholder="Телефон компании" value={phone} onChange={(e) => setPhone(e.target.value)} />
        <Input placeholder="Telegram (без @)" value={telegram} onChange={(e) => setTelegram(e.target.value)} />
